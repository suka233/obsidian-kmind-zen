import type { KmindApp, KmindFeature } from "@kmind/app";
import { parseKmindzProjectV3FromSvgText, unsafeGetKmindAppInternal, resolveProjectSaveFailure, translateProjectProtectionFailure } from "@kmind/app";
...
export function createObsidianFileSafetyFeature(args: { obsidianApp: App; file: TFile }): KmindFeature {
  return {
    ...
    async register(kmindApp) {
      ...
      const isProtected = () => {
        const state = kmindApp.host.ports.projectSaveIntegrity?.getState();
        return state?.mode === "quarantine" || Boolean(state?.retry);
      };
      const doReloadFromDisk = async () => {
        if (isProtected() || kmindApp.getSnapshot().documents.status.dirty) throw new Error(t("app.saveProtection.warning"));
      ...
      const doMergeFromDisk = async (diskText?: string | undefined) => {
        if (isProtected()) throw new Error(t("app.saveProtection.warning"));
      ...
      const doSaveAsCopy = async () => {
        if (!kmindApp.isCommandEnabled("document.exportUnsavedProject")) throw new Error(t("app.saveProtection.warning"));
        await kmindApp.dispatch("document.exportUnsavedProject");
      };
      ...
      const promptResolveConflict = async (reason: string) => {
        if (isProtected()) { new Notice(t("app.saveProtection.warning")); return; }
        if (handling) return;
        handling = true;
        try {
      ...
      const unsubscribeApp = kmindApp.subscribe(() => {
        ...
        const failure = resolveProjectSaveFailure(snapshot.documents.status.lastSaveFailure ?? saveError, kmindApp.host.ports.projectSaveIntegrity?.getState());
        new Notice(t("obsidian.fileSafety.notice.saveError", { message: translateProjectProtectionFailure(kmindApp.i18n.t, failure) }));
      });
