...
function parsePayloadFromJson(payloadJson: string): KmindZenObsidianLeasePayloadV1 | null {
  try {
    ...
    const entitlements = Array.isArray(parsed.entitlements)
      ? parsed.entitlements.map((x) => String(x ?? "").trim()).filter(Boolean)
      : [];
...
): KmindZenLicenseSnapshot["capabilities"] {
  const hasFoundersLimitedIcons = entitlements.includes("founders_limited_icons");
  return {
    tier: isPro ? "pro" : "free",
    ...
    canUseFoundersLimitedIcons: isPro && hasFoundersLimitedIcons,
  };
}
...
export function evaluateLicenseSnapshot(args: {
  ...
  return {
    ...
    reason: null,
    payload,
    capabilities: createCapabilities(true, payload.entitlements),
  };
}
