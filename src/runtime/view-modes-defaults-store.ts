...
export type CanvasWheelBehavior = "zoom" | "pan-with-modifier-zoom" | "disabled";
...
type StoredSettingsV1 = {
  ...
  canvasInteraction?: {
    ...
    wheelBehavior?: CanvasWheelBehavior | undefined;
...
function normalizeCanvasWheelBehavior(value: unknown): CanvasWheelBehavior {
  return value === "pan-with-modifier-zoom" || value === "disabled" || value === "zoom" ? value : "zoom";
}
...
function parse(text: string): KmindZenViewModesDefaults | null {
  try {
    ...
    const canvasInteraction = parsed.canvasInteraction as { dragMode?: unknown; wheelBehavior?: unknown; tipShown?: unknown } | null | undefined;
    ...
    const wheelBehavior = normalizeCanvasWheelBehavior(canvasInteraction?.wheelBehavior);
    return {
...
async function persist(app: App, next: KmindZenViewModesDefaults) {
  const payload: StoredSettingsV1 = {
    ...
    canvasInteraction: {
      ...
      wheelBehavior: next.wheelBehavior,
...
export const kmindZenViewModesDefaultsStore: ViewModesDefaultsStore & {
  ...
  set: async (app, next) => {
    const resolved: KmindZenViewModesDefaults = {
      ...
      wheelBehavior: normalizeCanvasWheelBehavior(next.wheelBehavior ?? current.wheelBehavior),
    ...
    if (
      ...
      && resolved.wheelBehavior === current.wheelBehavior
