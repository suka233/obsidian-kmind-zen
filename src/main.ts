...
import { recordObsidianSafetyRename } from "./host/obsidian/obsidian-safety-rename-links";
import { bootstrapKmindZenObsidianLicense } from "./runtime/license/license-bootstrap";
import { formatKmindZenLicensePlanLabel } from "./runtime/license/license-plan-label";
...
import {
  destroyKmindZenObsidianUpdateDialog,
  markKmindZenObsidianUpdateDialogCurrentVersionRead,
  maybeAutoOpenKmindZenObsidianUpdateDialog,
  openKmindZenObsidianUpdateDialog,
} from "./ui/obsidian-update-dialog-host";
...
class KmindZenSettingsTab extends PluginSettingTab {
  constructor(
    ...
    private readonly openUpdates: () => void,
  ...
  override display(): void {
    ...
    new Setting(containerEl)
      .setName(t("obsidian.settings.updates.label"))
      .setDesc(t("obsidian.settings.updates.desc"))
      .addButton((btn) => {
        btn.setButtonText(t("obsidian.settings.updates.button.open"));
        btn.onClick(() => {
          this.openUpdates();
        });
      });
    ...
    containerEl.createEl("h3", { text: t("obsidian.settings.section.canvasInteraction") });

    const canvasDragModeSetting = new Setting(containerEl)
      .setName(t("obsidian.settings.canvasInteraction.dragMode.label"))
      .setDesc(t("obsidian.settings.canvasInteraction.dragMode.desc"))
      .addDropdown((dropdown) => {
        dropdown.addOption("pan-first", t("obsidian.settings.canvasInteraction.dragMode.panFirst"));
        dropdown.addOption("select-first", t("obsidian.settings.canvasInteraction.dragMode.selectFirst"));
        dropdown.setValue(state.canvasDragMode);
        dropdown.onChange((value) => {
          void kmindZenViewModesDefaultsStore.set(this.app, { canvasDragMode: value === "select-first" ? "select-first" : "pan-first" });
        });
      });
    const canvasDragModeHelp = canvasDragModeSetting.descEl.createDiv({ cls: "kmind-zen-settings-option-help" });
    for (const item of [
      {
        label: t("obsidian.settings.canvasInteraction.dragMode.panFirst"),
        desc: t("obsidian.settings.canvasInteraction.dragMode.panFirstDesc"),
      },
      {
        label: t("obsidian.settings.canvasInteraction.dragMode.selectFirst"),
        desc: t("obsidian.settings.canvasInteraction.dragMode.selectFirstDesc"),
      },
    ]) {
      const row = canvasDragModeHelp.createDiv({ cls: "kmind-zen-settings-option-help__item" });
      row.createSpan({ cls: "kmind-zen-settings-option-help__label", text: item.label });
      row.appendText(`: ${item.desc}`);
    }

    const wheelBehaviorSetting = new Setting(containerEl)
      .setName(t("obsidian.settings.canvasInteraction.wheelBehavior.label"))
      .setDesc(t("obsidian.settings.canvasInteraction.wheelBehavior.desc"))
      .addDropdown((dropdown) => {
        dropdown.addOption("zoom", t("obsidian.settings.canvasInteraction.wheelBehavior.zoom"));
        dropdown.addOption("pan-with-modifier-zoom", t("obsidian.settings.canvasInteraction.wheelBehavior.panWithModifierZoom"));
        dropdown.addOption("disabled", t("obsidian.settings.canvasInteraction.wheelBehavior.disabled"));
        dropdown.setValue(state.wheelBehavior);
        dropdown.onChange((value) => {
          void kmindZenViewModesDefaultsStore.set(this.app, {
            wheelBehavior: value === "pan-with-modifier-zoom" || value === "disabled" ? value : "zoom",
          });
        });
      });
    const wheelBehaviorHelp = wheelBehaviorSetting.descEl.createDiv({ cls: "kmind-zen-settings-option-help" });
    for (const item of [
      {
        label: t("obsidian.settings.canvasInteraction.wheelBehavior.zoom"),
        desc: t("obsidian.settings.canvasInteraction.wheelBehavior.zoomDesc"),
      },
      {
        label: t("obsidian.settings.canvasInteraction.wheelBehavior.panWithModifierZoom"),
        desc: t("obsidian.settings.canvasInteraction.wheelBehavior.panWithModifierZoomDesc"),
      },
      {
        label: t("obsidian.settings.canvasInteraction.wheelBehavior.disabled"),
        desc: t("obsidian.settings.canvasInteraction.wheelBehavior.disabledDesc"),
      },
    ]) {
      const row = wheelBehaviorHelp.createDiv({ cls: "kmind-zen-settings-option-help__item" });
      row.createSpan({ cls: "kmind-zen-settings-option-help__label", text: item.label });
      row.appendText(`: ${item.desc}`);
    }
    ...
    containerEl.createEl("h3", { text: t("obsidian.settings.section.imageCompression") });

    new Setting(containerEl)
      .setName(t("obsidian.settings.imageCompression.enabled.label"))
      .setDesc(t("obsidian.settings.imageCompression.enabled.desc"))
      .addToggle((toggle) => {
        toggle.setValue(state.imageCompressionEnabled);
        toggle.onChange((value) => {
          void kmindZenViewModesDefaultsStore.set(this.app, { imageCompressionEnabled: value });
        });
      });
    ...
    const plan = formatKmindZenLicensePlanLabel(licenseSnapshot.payload, t("kmind.common.emptyDash"));
    const expiresAtText = (() => {
...
export default class KmindZenObsidianPlugin extends Plugin {
  ...
  private async maybeAutoOpenOnboardingGuide(): Promise<boolean> {
    if (this.isOnboardingSeen()) return false;
  ...
  override async onload(): Promise<void> {
    if (__KMIND_ZEN_DEV_BUILD_ID__) console.info("%c[kmind-obsidian] build id: " + __KMIND_ZEN_DEV_BUILD_ID__, "color: #16a34a; font-weight: bold");
    ...
    if (kmindZenViewModesDefaultsStore.shouldShowCanvasInteractionTip()) {
      new Notice(t("obsidian.notice.canvasInteractionTip"), 5000);
      void kmindZenViewModesDefaultsStore.markCanvasInteractionTipShown(this.app);
    }
    ...
    this.addSettingTab(new KmindZenSettingsTab(
      this.app,
      this,
      uiI18n,
      t,
      () => this.openOnboardingGuide(),
      () => openKmindZenObsidianUpdateDialog(this.app),
      () => this.openThemeDesigner(),
    ));
    ...
    this.registerEvent(this.app.vault.on("rename", (file, oldPath) => {
      const movedRoot = file.path;
      const record = (entry: TAbstractFile): void => {
        if (entry instanceof TFolder) { for (const child of entry.children) record(child); return; }
        if (!(entry instanceof TFile) || !(entry.path.endsWith(".kmindz") || entry.path.endsWith(".kmindz.svg"))) return;
        const previous = entry === file ? oldPath : oldPath + entry.path.slice(movedRoot.length);
        void recordObsidianSafetyRename(this.app, previous, entry.path).catch(error => {
          new Notice(`KMind: ${error instanceof Error ? error.message : String(error)}`);
        });
      };
      record(file);
    }));
    ...
    this.app.workspace.onLayoutReady(() => {
      void this.maybeAutoOpenOnboardingGuide()
        .then((openedOnboarding) => {
          if (openedOnboarding) {
            markKmindZenObsidianUpdateDialogCurrentVersionRead(this.app);
            return;
          }
          maybeAutoOpenKmindZenObsidianUpdateDialog(this.app);
        })
        .catch((error) => {
