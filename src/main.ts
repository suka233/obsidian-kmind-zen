import { Menu, Notice, Plugin, PluginSettingTab, Setting, TFile, TFolder, normalizePath, type TAbstractFile } from "obsidian";
...
export default class KmindZenObsidianPlugin extends Plugin {
  ...
  private resolvePluginAssetUrl(relativePath: string): string {
    const pluginDir = this.manifest.dir || `${this.app.vault.configDir}/plugins/${this.manifest.id}`;
    const normalized = normalizePath(`${pluginDir}/${relativePath}`);
  ...
  private openOnboardingGuide(): void {
    ...
    this.onboardingModal = openKmindZenObsidianOnboardingModal({
      ...
      iconSvg: rawIconSvg,
      media: {
        commandPalette: this.resolvePluginAssetUrl("onboarding/obsidian-command.webp"),
        folderMenu: this.resolvePluginAssetUrl("onboarding/obsidian-folder-menu.webp"),
        openFile: this.resolvePluginAssetUrl("onboarding/obsidian-open-file.webp"),
        zen: this.resolvePluginAssetUrl("onboarding/obsidian-zen.webp"),
      },
      onClose: () => {
