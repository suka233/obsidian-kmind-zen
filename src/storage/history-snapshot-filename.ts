export type HistoryCheckpointTag = "auto" | "manual" | "before-restore" | "before-import" | "before-migration" | "before-destructive-save" | "unknown";
...
function resolveTagFromFileName(fileName: string, rest: string): HistoryCheckpointTag {
  ...
  if (tag === "before-destructive-save") return "before-destructive-save";
  if (tag === "before-migration") return "before-migration";

  const name = String(fileName ?? "");
  if (name.includes("before-import") || name.includes("before_import")) return "before-import";
  if (name.includes("before-destructive-save") || name.includes("before_destructive_save")) return "before-destructive-save";
      ...
      : rawTag === "auto"
        ...
        : rawTag === "before_restore" || rawTag === "before-restore"
          ...
          : rawTag === "before_import" || rawTag === "before-import"
            ? "before-import"
            : rawTag === "before_migration" || rawTag === "before-migration"
              ? "before-migration"
  ...
  const tag = args.tag === "unknown"
    ...
    : args.tag === "before-restore"
      ...
      : args.tag === "before-import"
        ? "before_import"
        : args.tag === "before-migration"
          ? "before_migration"
        : args.tag;
  const tokens: string[] = [String(args.ts), tag];
  if (args.nonce) {
    if (!/^[a-zA-Z0-9-]+$/u.test(args.nonce)) throw new Error("Invalid checkpoint nonce.");
    tokens.push(`id-${args.nonce}`);
  }
