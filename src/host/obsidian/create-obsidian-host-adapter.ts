  ...
  const store = {
    ...
    restoreFromYjsUpdateInternal: async (args2: { update: Uint8Array }) => {
      ...
      await enqueueWrite(async () => {
        ...
        const create = restoreContext.collab.createYDoc;
        ...
        const replaceProjectSnapshot = restoreContext.collab.replaceProjectSnapshotInCrdt;
        if (
          typeof listDocIds !== "function" ||
          typeof replaceProjectSnapshot !== "function"
        ) {
        ...
        const nextDocs: Record<DocumentId, DocumentRecord> = {};
        try {
          restoreContext.collab.applyYDocUpdate(tmp, args2.update, { kind: "kmind-yjs:restore-prepare" });
          ...
          preparedUpdate = restoreContext.collab.encodeYDocStateAsUpdate(tmp);
