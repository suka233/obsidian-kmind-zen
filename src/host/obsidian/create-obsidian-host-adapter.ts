...
function createObsidianProjectAssetStore(args: {
  ...
  return {
    ...
    async get(id) {
      ...
      const bytes = await readObsidianBinaryFile(args.app, path);
      if (bytes) {
...
function createProjectDocumentStore(args: {
  ...
  const preflightProjectAssets = async (records: Iterable<DocumentRecord>, rootDocId: DocumentId): Promise<void> => {
    ...
    await unusedAssetPolicy.validate({
      ...
      async readExternalAsset(request) {
        ...
        const bytes = await readObsidianBinaryFile(args.app, path, { suppressErrors: false });
        return { bytes, path };
      },
    });
  };
  ...
  const hydrateFromPayload = async (payload: KmindzProjectSvgPayloadV3, text: string) => {
    ...
    const maintenance = await cleanupObsidianCompletedTransactions({
      ...
      inspectSource: async sourceText => {
        const sourcePayload = parseKmindzProjectV3FromSvgText(sourceText);
  ...
  const buildAndWriteProjectFile = async (options?: {
    ...
    if (options?.schedulePreview !== false) {
    ...
    async delete(id: DocumentId) {
      await enqueueWrite(async () => {
        ...
        if (rootDocId && id === rootDocId) {
          ...
          const payload = parseKmindzProjectV3FromSvgText(sourceText);
