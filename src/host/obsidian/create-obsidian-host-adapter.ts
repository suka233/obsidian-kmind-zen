...
import {
  assertProjectExternalAssetsDurable,
...
async function writeProjectTextAtRevision(args: {
  ...
  await writeProjectTextAtRevisionWithProcess({
    ...
    readRevision: (currentText) => (
      currentText.trim()
        ? parseKmindzProjectV3FromSvgText(currentText)?.header.rev ?? null
        : null
    ),
...
function createProjectDocumentStore(args: {
  ...
  const preflightProjectAssets = async (records: Iterable<DocumentRecord>, rootDocId: DocumentId): Promise<void> => {
    await assertProjectExternalAssetsDurable({
      records,
  ...
  const previewRefreshCoordinator = createRevisionPreviewRefreshCoordinator({
    refresh: async (request) => {
      ...
      if (!previewSvg) return { previewSvg: "", applied: false };

      let applied = false;
      await enqueueWrite(async () => {
        const currentText = await readText(args.app, args.file).catch(() => "");
        ...
        const historyResults = await Promise.allSettled(
          Array.from(request.historyPreviewPaths, (path) => args.app.vault.adapter.write(path, previewSvg)),
        );
    ...
    async reuseCompleted({ result, historyPreviewPaths }) {
      await Promise.all(
        Array.from(historyPreviewPaths, (path) => args.app.vault.adapter.write(path, result.previewSvg)),
      );
    },
    ...
    restoreFromYjsUpdateInternal: async (args2: { update: Uint8Array }) => {
      ...
      await enqueueWrite(async () => {
        ...
        const written = await commitDurableProjectRestore({
          persistCanonical: () => buildAndWriteProjectFile({
            preparedSnapshot: {
              documents: nextDocs,
