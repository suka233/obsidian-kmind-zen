...
function createObsidianProjectAssetStore(args: {
  ...
  return {
    ...
    async put(record) {
      ...
      const ext = extFromMimeType(record.mimeType);
      const contentHash = await sha256Hex(record.bytes);
      const entry: KmindzSvgAssetEntryWithSizeV1 = { mimeType: record.mimeType, contentHash, ext, size: record.bytes.length };
      const path = resolveAssetFile(rootDocId, entry);
      await writeBinaryFileIfMissing(path, record.bytes);
      args.assetsIndex.upsert(record.id, { entry });
      memoryById.set(record.id, { mimeType: record.mimeType, bytes: record.bytes });
      return { storage: { kind: "external-v1", contentHash, ext, size: record.bytes.length } as const };
    },
    async delete(id) {
      await args.ensureLoaded();
