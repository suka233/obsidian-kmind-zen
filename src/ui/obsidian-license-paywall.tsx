...
function resolveStrikePriceCents(offer: ObsidianPaywallScene["offers"][number]): number | null {
  if (offer.couponApplied && offer.originalAmountCents > offer.amountCents) {
    return offer.originalAmountCents;
  }
  if (typeof offer.compareAtCents === "number" && offer.compareAtCents > offer.amountCents) {
    return offer.compareAtCents;
  }
  return null;
}
...
function purchaseResultItems(
  session: ObsidianPurchaseSessionSnapshot | null,
): Array<{ label: string; value: string }> {
  const result = session?.result;
  if (!result) return [];
  if (result.kind === "web") return [{ label: "Backup key", value: result.backupLicenseKey }];
  if (result.kind === "obsidian_plugin") return [{ label: "Obsidian key", value: result.backupLicenseKey }];
  if (result.kind === "founders_pass") return [{ label: "Founders Pass", value: result.foundersPassCode }];
  return result.components.flatMap((component) => {
    if (component.kind === "license_key") {
      return [{ label: `${component.product} key`, value: component.backupLicenseKey }];
    }
    if (component.kind === "legacy_activation_code") {
      return [{ label: "Legacy SiYuan code", value: component.legacyActivationCode }];
    }
    return [{ label: "Founders Pass", value: component.foundersPassCode }];
  });
}

function resolveAutoObsidianSession(session: ObsidianPurchaseSessionSnapshot | null) {
  const result = session?.result;
  if (!result) return null;
  if (result.kind === "obsidian_plugin") {
    return { lease: result.lease, refreshToken: result.refreshToken };
  }
  if (result.kind !== "bundle") return null;
  const component = result.components.find(
    (item) =>
      item.kind === "license_key" &&
      (item.product === "obsidian_plugin" || item.product === "universal") &&
      item.lease &&
      item.refreshToken,
  );
  if (component?.kind !== "license_key" || !component.lease || !component.refreshToken) return null;
  return { lease: component.lease, refreshToken: component.refreshToken };
}
...
export function ObsidianLicensePaywall(props?: {
  ...
  const purchaseItems = useMemo(
    () => purchaseResultItems(purchaseSession),
    [purchaseSession],
  );
  ...
  const purchaseWebsiteUrl = useMemo(() => resolvePurchaseWebsiteUrl(purchaseLocale), [purchaseLocale]);

  useEffect(() => {
    if (scene?.couponGuide?.text) return;
    setCouponGuideOpen(false);
  }, [scene?.couponGuide?.text]);
  ...
  useEffect(() => {
    ...
    const autoSession = resolveAutoObsidianSession(purchaseSession);
  ...
  async function applyCoupon() {
    ...
    try {
      ...
      const res = await apiQuotePaywallScene({
    ...
    <div style={{ display: "flex", width: "100%", height: "100%", alignItems: "center", justifyContent: "center" }}>
      <div style={{ width: 680, maxWidth: "calc(100% - 32px)", borderRadius: 16, padding: 16, border: "1px solid rgba(148,163,184,.35)" }}>
        ...
        <div style={{ marginTop: 6, fontSize: 13, color: "#64748b", lineHeight: 1.6 }}>
          ...
          <span style={{ fontFamily: "monospace" }}>{formatKmindZenLicensePlanLabel(snapshot.payload, t("kmind.common.emptyDash"))}</span>
        ...
        {purchaseOpen ? (
          <div style={{ marginTop: 16, display: "grid", gap: 12 }}>
            <a
              href={purchaseWebsiteUrl}
              rel="noreferrer"
              target="_blank"
              style={{ display: "inline-flex", width: "fit-content", height: 34, alignItems: "center", padding: "0 12px", borderRadius: 999, border: "1px solid rgba(15,118,110,.35)", color: "#0f766e", fontSize: 12, fontWeight: 700, textDecoration: "none" }}
            >
              {t("obsidian.paywall.purchase.websiteAction")}
            </a>

            <div style={{ display: "grid", gap: 8, borderRadius: 14, border: "1px solid rgba(148,163,184,.28)", padding: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ fontSize: 12, color: "#475569" }}>{t("obsidian.paywall.purchase.couponLabel")}</div>
                {scene?.couponGuide?.text ? (
                  <button
                    type="button"
                    aria-expanded={couponGuideOpen}
                    aria-label={t("obsidian.paywall.purchase.couponGuide")}
                    onClick={() => setCouponGuideOpen((value) => !value)}
                    style={{
                      width: 22,
                      height: 22,
                      borderRadius: 999,
              ...
              {scene?.couponGuide?.text && couponGuideOpen ? (
                <div
                  role="tooltip"
                  style={{
                    borderRadius: 12,
                ...
                >
                  <div>{scene.couponGuide.text}</div>
                  {scene.couponGuide.href ? (
                    <a
                      href={scene.couponGuide.href}
                      rel="noreferrer"
                      target="_blank"
                      style={{ display: "inline-flex", marginTop: 8, color: "#0f766e", fontWeight: 700 }}
                    >
                      {t("obsidian.paywall.purchase.couponGuideLink")}
                    </a>
                  ) : null}
                </div>
              ) : null}
              <div style={{ display: "flex", gap: 8 }}>
                <input
                  ...
                  placeholder={t("obsidian.paywall.purchase.couponPlaceholder")}
                ...
                <button
                  ...
                  style={{ height: 36, padding: "0 12px", borderRadius: 10, border: "1px solid rgba(148,163,184,.35)" }}
                >
                  {couponBusy ? "…" : t("obsidian.paywall.purchase.couponApply")}
                </button>
              </div>
              {scene?.coupon ? (
            ...
            {(scene?.offers ?? []).map((offer) => {
              const strikePriceCents = resolveStrikePriceCents(offer);
              return (
                <div
                  key={offer.id}
                  style={{
                ...
                >
                  ...
                  {offer.couponApplied ? (
                    <div style={{ marginTop: 8, fontSize: 12, color: "#047857" }}>
                      {offer.couponLabel ?? t("obsidian.paywall.purchase.couponApplied")}
                    </div>
                  ) : null}
                  {!offer.quantityAvailable ? (
                    <div style={{ marginTop: 8, fontSize: 12, color: "#b45309" }}>
                      {offer.quantityUnavailableReason ?? t("obsidian.paywall.purchase.quantityUnavailable")}
                    </div>
                  ) : null}
                  ...
                  <div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {offer.providers.map((provider) => (
                      <button
                        key={provider}
                        type="button"
                        disabled={busy || !offer.quantityAvailable}
                        onClick={() => run(async () => {
                          if (!offer.quantityAvailable) {
                            throw new Error(offer.quantityUnavailableReason ?? t("obsidian.paywall.purchase.quantityUnavailable"));
                          }
                          const normalizedEmail = normalizeEmail(email);
                          if (!isValidEmail(normalizedEmail)) throw new Error(t("obsidian.error.invalidEmail"));
                          await kmindZenObsidianLicenseStore.ensureLoaded();
                ...
                {purchaseItems.length > 0 ? (
                  <div style={{ marginTop: 10, display: "grid", gap: 6 }}>
                    {purchaseItems.map((item) => (
                      <div key={`${item.label}:${item.value}`} style={{ fontSize: 12, color: "#475569", whiteSpace: "pre-wrap" }}>
