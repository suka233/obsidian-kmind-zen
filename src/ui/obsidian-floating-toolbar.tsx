...
function DocumentExportPopover(props: { app: KmindApp; snapshot: KmindAppSnapshot; onClose: () => void }) {
  ...
  const behavior = useMemo(
    () => resolveDocumentExportBehavior(kind, scope, kind === "png" ? { beautify, scale: pngScale } : undefined),
    [beautify, kind, pngScale, scope],
  );
  const args = behavior.svgArgs ?? behavior.pngArgs ?? undefined;
    ...
    <>
      ...
      <div className="kmind-zen-popover__body">
        <div className="kmind-zen-field">
          ...
          <select className="kmind-zen-select" onChange={(event) => setKind(event.target.value as ExportKind)} value={kind}>
            <option value="svg-package">{t("appPage.popover.export.type.svgPackage")}</option>
            <option value="kmindz">{t("appPage.popover.export.type.kmindz")}</option>
            <option value="png-package">{t("appPage.popover.export.type.pngPackage")}</option>
            <option value="zip">{t("appPage.popover.export.type.zip")}</option>
            <option value="svg">{t("appPage.popover.export.type.svg")}</option>
            <option value="png">{t("appPage.popover.export.type.png")}</option>
          </select>
        </div>
    ...
    <div ref={rootRef} className={["kmind-zen-floating", isCompact ? "kmind-zen-floating--compact" : ""].filter(Boolean).join(" ")}>
          ...
          if (isToolbarMenu(item)) {
            ...
            return (
              <div key={item.id} style={{ position: "relative" }}>
                ...
                {openPopover === item.id ? (
                  <div className="kmind-zen-popover" style={{ left: 0, top: "calc(100% + 8px)", width: 280, overflow: "hidden" }}>
      ...
      <div className={["kmind-zen-bar", "kmind-zen-bar--bottom", zenMode ? "kmind-zen-bar--hidden" : ""].filter(Boolean).join(" ")}>
        ...
        <div style={{ position: "relative" }}>
          ...
          {openPopover === "minimap" ? (
            <div className="kmind-zen-popover" style={{ right: 0, bottom: "calc(100% + 8px)", width: 320, overflow: "hidden" }}>
