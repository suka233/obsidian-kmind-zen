...
import type { KmindAppSnapshot, MindMapCanvasPort, MindMapExportPngArgs, MindMapExportSvgArgs, ThemeCatalogItem } from "@kmind/app";
import { THEME_PRESETS_APP, createOfficialThemeCatalogItems, listLocalThemeCatalogItems, resolveEffectiveKeymap, resolveKeymapMatch, resolveProjectBackgroundColorFromDoc } from "@kmind/app";
import {
  KMIND_WORKSPACE_ALL_VIEW_MODES,
  KMIND_WORKSPACE_MAP_ONLY_VIEW_MODES,
...
const KMIND_OBSIDIAN_ENABLE_OUTLINE_WORKSPACE_MODES = false;
const KMIND_OBSIDIAN_AVAILABLE_WORKSPACE_VIEW_MODES = KMIND_OBSIDIAN_ENABLE_OUTLINE_WORKSPACE_MODES
  ? KMIND_WORKSPACE_ALL_VIEW_MODES
  : KMIND_WORKSPACE_MAP_ONLY_VIEW_MODES;
...
export function KmindObsidianViewApp(props: { hostApp: App; file: TFile | null }) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (!current) return;
      const bindings = resolveEffectiveKeymap(current.registry.keymap, keymapOverrides);
  ...
  const renderMap = (_visible = true, crossPaneDrag?: MindMapCrossPaneDragBridge) => (
    <MindMapCanvas
      ...
      dragGhostMode="rich"
      nodeChrome={{
        showNodeWidthResizeHandle: false,
    ...
    <I18nProvider i18n={runtime.i18n}>
      ...
      >
        <KmindOutlineWorkspace
          ...
          selectInCanvas={(target) => {
            if (target.type === "node") canvasApiRef.current?.selectNode(target.id);
            else canvasApiRef.current?.selectSummary(target.id);
          }}
          snapshot={snapshot}
        />
      </div>
